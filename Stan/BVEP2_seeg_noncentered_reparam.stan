...
model {
  ...
  } else if (ode_solver == 5) {
    vector[2*nn] ic = append_row(x_init, z_init);
    real it = 0.0;
    real ts[nt-1];
    for (t in 2:nt) ts[t-1] = t*dt;
    vector[2*nn] ode_sol[nt-1] = ode_ckrk_tol(ode_rhs, ic, it, ts, 
    ode_rtol, ode_atol, ode_maxstep, 
    SC_star, I1, tau0, K, eta);
    for (t in 2:nt) x[,t] = ode_sol[t-1][1:nn];
  } else if (ode_solver == 6) {
    vector[2*nn] ic = append_row(x_init, z_init);
    real it = 0.0;
    real ts[nt-1];
    for (t in 2:nt) ts[t-1] = t*dt;
    vector[2*nn] ode_sol[nt-1] = ode_adjoint_tol_ctl(ode_rhs, ic, it, ts, 
    ode_rtol, ode_atol_fwd, ode_rtol, ode_atol_fwd, ode_rtol, ode_atol, ode_maxstep, 100, 1, 1, 1, 
    SC_star, I1, tau0, K, eta);
    for (t in 2:nt) x[,t] = ode_sol[t-1][1:nn];
  }
