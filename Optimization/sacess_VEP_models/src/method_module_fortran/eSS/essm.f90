...
CONTAINS
    ...
    FUNCTION essm(exp1, fitnessfunction, results1, maxfunevals, ftarget) RESULT (outresult)
        ...
        CALL hete_param_eSS(exp1, problem1, opts1, common_vars%NPROC, opts1%globaloptions%dim_refset,&
...
!----------------------------------------------------------------------------------------------------------------------------------              
            ...
            cputime1 = calctimeMPI(exp1,time%starttime)
            CALL asynchronousstoppingcriteriaess(exp1, fin, nfuneval, fbest, cputime1, opts1%useroptions%maxtime,&
            stopOptimization)
...
!----------------------------------------------------------------------------------------------------------------------------------
            ...
            if ((opts1%localoptions%empty .eq. 0 ) .and. (fin .eq. 0)) then
              ...
              F02=fbest(1)
              !if (common_vars%idp .NE. 0) CALL printiterationcess(exp1, F02, nfuneval,cputime1, 1, 0) 
