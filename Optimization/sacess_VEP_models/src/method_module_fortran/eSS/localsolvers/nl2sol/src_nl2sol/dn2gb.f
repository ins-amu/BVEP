      SUBROUTINE  DN2GB(N, P, X, B, IV, LIV, LV, V,
     1    UIPARM, URPARM, DATAAMIGO, NPROC, fitnessfunction)
...
C
      ...
      TYPE(C_PTR), INTENT(INOUT) ::  DATAAMIGO
      INTEGER(C_INT), INTENT(INOUT) :: NPROC
      INTEGER(C_INT) :: STOP1
      INTEGER(C_INT), INTENT(INOUT) ::  N, P, LIV, LV
C/6
C     INTEGER IV(LIV), UIPARM(1)
C     DOUBLE PRECISION X(P), B(2,P), V(LV), URPARM(1)
C/7
      INTEGER(C_INT), INTENT(INOUT) :: IV(LIV), UIPARM(*)
      REAL(C_DOUBLE), INTENT(INOUT) :: X(P), B(2,P), V(LV), URPARM(*)
...
C
 30   NF = IV(NFCALL)
      CALL CALCR(N, P, X, NF, V(R1), UIPARM, URPARM, DATAAMIGO, 
     1             STOP1, NPROC, fitnessfunction)
      IF (STOP1 .EQ. 1) GO TO 999
      IF (NF .GT. 0) GO TO 40
         IV(TOOBIG) = 1
         GO TO 20
...
C
 50   CALL CALCJ(N, P, X, IV(NFGCAL), V(DR1), UIPARM, URPARM, 
     1            DATAAMIGO, STOP1,  NPROC, fitnessfunction)
      IF (STOP1 .EQ. 1) GO TO 999
