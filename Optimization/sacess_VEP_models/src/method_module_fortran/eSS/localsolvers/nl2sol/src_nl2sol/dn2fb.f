      SUBROUTINE  DN2FB(N, P, X, B, IV, LIV, LV, V, UI, UR, 
     1                      DATAAMIGO, NPROC, fitnessfunction)
C
...
C/7
      EXTERNAL CALCR
      INTEGER(C_INT), INTENT(INOUT) :: IV(LIV), UI(*)
      REAL(C_DOUBLE), INTENT(INOUT) ::  X(P), B(2,P), V(LV), UR(*)
...
C
 30   NF = IV(NFCALL)
      CALL CALCR(N, P, X, NF, V(R1), UI, UR, DATAAMIGO, STOP1,
     1        NPROC, fitnessfunction )
...
C
 ...
 70      X(K) = XK1
         NF = IV(NFGCAL) 
         CALL CALCR(N, P, X, NF, V(J1K), UI, UR, DATAAMIGO, STOP1,
     1        NPROC, fitnessfunction)     
         IF (STOP1 .EQ. 1) GO TO 999
         NG = NG + 1
         IF (NF .GT. 0) GO TO 90
