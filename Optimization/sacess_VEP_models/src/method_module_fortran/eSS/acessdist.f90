...
CONTAINS
    ...
    FUNCTION acessdist(exp1, fitnessfunction, results1, maxfunevals, ftarget) RESULT (outresult)
        ...
        INTEGER :: stopOptimization, fin, nreset, l
        INTEGER(C_LONG) :: nfuneval, nfunevaltotal
        INTEGER :: i, j, nconst, dest
        REAL(C_DOUBLE) :: cputime1
        INTEGER :: clock_rate, clock_start
        ...
        INTEGER :: flag , cooperativempitestess
        REAL(C_DOUBLE) ::  initmpi, calctimempi
        REAL(KIND=SELECTED_REAL_KIND(P=PRECISION_D,R=RANGE_D)), DIMENSION(:,:), ALLOCATABLE :: last_recv_x
        REAL(KIND=SELECTED_REAL_KIND(P=PRECISION_D,R=RANGE_D)), DIMENSION(:), ALLOCATABLE :: last_recv_f
        CALL setnan(NAN)
        CALL setdblmax(INF)
        ...
        ALLOCATE(init(common_vars%NPROC))
        init = 0
        ...
        CALL initoldbestvars(last_recv_x,last_recv_f,common_vars%nvar)
        ...
        CALL calc_dim_refset(opts1%globaloptions%dim_refset, common_vars%nvar, opts1%useroptions%iterprint,common_vars%idp,&
                                                                common_vars%NPROC) 
        CALL hete_param_eSS(exp1, problem1, opts1, common_vars%NPROC, opts1%globaloptions%dim_refset, common_vars%idp, &
                                                                common_vars%nvar)
        ...
        CALL slave_information(common_vars%idp, opts1%globaloptions%dim_refset, opts1%globaloptions%ndiverse, 1)
...
!-------------------------------------------------------------------------------------------------------!  
        CALL asynchinitasynchronousvars( exp1, common_vars%sizeser )     
...
!-------------------------------------------------------------------------------------------------------!
        ...
        i=1
        j=1
        hyper_x_L = reshape((/ ((problem1%XL(i), i = 1,common_vars%nvar), j = 1, &
        CEILING(MaxSubSet)) /), (/ common_vars%nvar, CEILING(MaxSubSet) /))
        hyper_x_U = reshape((/ ((problem1%XU(i), i = 1,common_vars%nvar), j = 1, &
        CEILING(MaxSubSet)) /), (/ common_vars%nvar, CEILING(MaxSubSet) /))
...
!----------------------------------------------------------------------------------------------------------------------------------   
             ...
             if (common_vars%iter .NE. 0) then
                CALL asynchronous_acess_dist (exp1,opts1,problem1,refset,common_vars%sizeser,common_vars%nvar,nconst,&
                                                                last_recv_x,last_recv_f,fbest,xbest, init, &
                                                                common_vars%idp,time%starttime,refset_change)
             else
                last_recv_f(1) = fbest(1)
                last_recv_x(:,1) = xbest
             end if
!----------------------------------------------------------------------------------------------------------------------------------   
...
!---------------------------------------------------------------------------------------------------------             
            ...
            CALL update_candidateset_with_new_comb_parallel( exp1,opts1, fitnessfunction,problem1,new_comb,candidateset,&
                childset,candidate_update, members_update,CEILING(MaxSubSet2),nrand,nconst,nfuneval,counter, index )
            else 
...
!----------------------------------------------------------------------------------------------------------------------------------
           ...
           CALL apply_beyond_to_members_to_update( exp1, opts1, fitnessfunction, problem1, members_to_update,  nfuneval, &
            ...
            if ((opts1%useroptions%iterprint .eq. 1) .AND.( MOD(common_vars%iter,10).EQ. 1) ) then
                cputime1 = calctimeMPI(exp1,time%starttime)
                
                WRITE(*, '(A13, I3, A16, I10, A12, I10, A8, F25.10, A10, F12.2, A12, E10.1)') &
                "[PROCESSOR ID=", common_vars%idp, &
                "] - Iterations: ", common_vars%iter,&             
                " NFunEvals: ", nfuneval, & 
                " Bestf: ", fbest(1), " CPUTime: ", cputime1, " variance: ", variance_vector(refset%fpen)
            end if              
...
!----------------------------------------------------------------------------------------------------------------------------------              
            ...
            if (fin > 0) then
                cputime1 = calctimeMPI(exp1,time%starttime)        
                F02 = fbest(1)
                CALL printiterationcess(exp1, F02,nfuneval,cputime1, 0, 0)
                CALL printgant(exp1,cputime1,1)         
            end if
...
!---------------------------------------------------------------------------------------------------------
        ...
        CALL  returnavgelementintdouble( exp1, common_vars%iter, results%totaliter, common_vars%NPROC)
...
!---------------------------------------------------------------------------------------------------------
        ...
        if (ALLOCATED(last_recv_x)) DEALLOCATE(last_recv_x)
        if (ALLOCATED(last_recv_f)) DEALLOCATE(last_recv_f)
    ...
    END FUNCTION acessdist
#endif

    
END MODULE modacessdist
